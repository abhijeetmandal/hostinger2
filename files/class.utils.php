<?php
    class utils {
        ...
        public function timeSince2($date, $short=false, $forceSince=false) {
            $date = strtotime($date);
            ...
            //echo "isSameDay: ".$isSameDay."<br/>";
            //echo "forceSince: ".$forceSince."<br/>";
            //echo "short: ".$short."<br/>";

            if ($isSameDay || $forceSince) {
                if ($diff < 60)
                    return "secs" . (!$short?' ago (Today at '.date('h:i A', $date).')':'');
                else if ($diff < 3600) {
                    $n = floor($diff/60);
                    return "{$n} min" . ($n==1?'':'s') . (!$short?' ago (Today at '.date('h:i A', $date).')':''); 
                } else if ($diff < 86400) {
                    $n = floor($diff/3600);
                    return "{$n} hour" . ($n==1?'':'s') . (!$short?' ago (Today at '.date('h:i A', $date).')':''); 
                } else if ($diff < 604800) {
                    $n = floor($diff/86400);
                    return "{$n} day" . ($n==1?'':'s') . (!$short?' ago (Today at '.date('h:i A', $date).')':''); 
                } else if ($diff < 2630000) {
                    $n = floor($diff/604800);
                    return "{$n} week" . ($n==1?'':'s') . (!$short?' ago (Today at '.date('h:i A', $date).')':''); 
                } else if ($diff < 31560000) {
                    $n = floor($diff/2630000);
                    return "{$n} month" . ($n==1?'':'s') . (!$short?' ago (Today at '.date('h:i A', $date).')':''); 
                } else {
                    $n = floor($diff/31560000);
                    return "{$n} year" . ($n==1?'':'s') . (!$short?' ago (Today at '.date('h:i A', $date).')':''); 
                }
            } else if ($short) {
