<?php
    class levels {
        ...
        public function getLevel($group, $name, $noSkip=false) {
            ...
            if ($cache):
                ...
                $sql = "SELECT
                            ...
                            users_levels.submitted as `submitted_time`,
                ...
                $sql = "SELECT levels.level_id, levels.name, levels_groups.title AS `group`, CONCAT(`group`, ' Level ', levels.name) as `title`,
                            IF(users_levels.completed > 0, 1, 0) as `completed`, users_levels.completed as `completed_time`,IF(users_levels.submitted > 0, 1, 0) as `submitted`, users_levels.submitted as `submitted_time`, `started`,
                            IFNULL(users_levels.attempts, 0) as `attempts`,
                ...
                // Get stats
                $sql = "SELECT COUNT(user_id) AS `completed` FROM users_levels WHERE completed > 0 AND level_id = :lid AND user_id != 8";
                $st = $this->app->db->prepare($sql);
                $st->execute(array(':lid'=>$level->level_id));
                $result = $st->fetch();
                $level->count = $result->completed;
                
                $sql = "SELECT COUNT(user_id) AS `submitted` FROM users_levels WHERE submitted > 0 AND level_id = :lid AND user_id != 8";
                $st = $this->app->db->prepare($sql);
                $st->execute(array(':lid'=>$level->level_id));
                $result = $st->fetch();
                $level->count_submitted = $result->submitted;
        ...
        function check($level) {
            //$this->app->log->add("users","levels class");
            ...
            foreach($answers AS $answer) {
                ...
                if (strtolower($answer->method) == 'post') {
                    //$this->app->log->add("users","name: ".$answer->name.", post name value: ".$_POST[$answer->name]);
                    //echo "post name: ". $_POST[$answer->name] ."<br/>";
                    //echo "DB value: ". $answer->value ."<br/>";
                    if (isset($_POST[$answer->name])) {
                        $attempted = true;
                        if (isset($answer->type) && $answer->type == 'regex') {
                            //echo "value: ".$answer->value."</br>";
                            //echo "name: ".$_POST[$answer->name]."</br>";
...
//echo "attempted: ".$attempted."<br/>";
        ...
        function attempt($level, $correct=false) {
            ...
            if (!$level->completed) {
                ...
                $level->submitted_time = 'now';
                if ($correct) {
                    $level->submitted = true;
                    $level->count_submitted++;
                    $level->last_user = $this->app->user->username;
                    $level->last_submitted = "now";
                   
                    //Update user score (temporary)
                    $this->app->user->score = $this->app->user->score + $level->data['reward'];
                    $st = $this->app->db->prepare('UPDATE users_levels SET submitted = NOW(), attempts=attempts+1 WHERE level_id = :lid AND user_id = :uid');
                    $st->execute(array(':lid'=> $level->level_id, ':uid' => $this->app->user->uid));

                    // Setup GA event
                    $this->app->ssga->set_event('level', 'submitted', $level->level_id, $this->app->user->uid);
                    $this->app->ssga->send();
        ...
        function completed($level, $correct=false) {
            if (!$level->completed) {
                $level->attempts = $level->attempts + 1;
