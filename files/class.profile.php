<?php
    class profile {
        ...
        public function __construct($username, $public=false) {
            ...
            //$log = new log($this);
            //$log->add("users", "class.profile.php construct");

            if ($public) {
                $st = $this->app->db->prepare("SELECT u.user_id as uid, u.username, u.score, u.email, u.mobile, profile.show_email,profile.show_mobile, profile.about, profile.forum_signature,
                ...
                // is this user allowed to see that stuff?
                if (!$this->app->user->admin_site_priv && !$this->show_mobile) {
                    unset($this->mobile);
                }
                unset($this->show_mobile);
                ...
                $st = $this->app->db->prepare("SELECT u.user_id as uid, u.username, u.score, u.email,u.mobile, profile.*, activity.joined,
        ...
        public function addFriend() {
            //$this->app->log->add("users.log","class.profile.php , inside addFriend()");
            //$log = new log(this);
            //$log.add("users.log","add friends");
            //echo "hello1";
        ...
        public static function getImg($img, $size, $gravatar=false) {
            ...
            $default = "https://crushit.fit/users/images/{$size}/1:1/no_pic.jpg";
            ...
            if ($gravatar) {
                ...
                return "https://crushit.fit/users/images/{$size}/1:1/{$img}";
            }
        }
    }
?>
