...
<?php if (!($level->submitted && !$level->attempts)): ?>	
	<span class='dark'>Attempts: <?=$level->attempts;?>
<?php 	if ($level->submitted): ?>
		&middot; Duration: <?=$app->utils->timeBetween($level->started, $level->submitted_time);?>
<?php 	endif; ?>
	</span>
...
<?php endif; ?>
	<span class='strong <?=$level->completed?'green':($level->submitted?'yellow':'red');?>'><?=($level->completed)?'Completed':($level->submitted?'Submitted':'Incomplete');?></span><br/>
	...
	if (isset($level->attempt)) {
		if ($level->attempt === true)
			//$app->utils->message('Level complete'.(isset($level->level_after_uri)?", <a href='$level->level_after_uri'>next level</a>":''), 'good');
                    $app->utils->message('Level Submitted'.(isset($level->level_after_uri)?", <a href='/userlevelqueue.php'>check status</a>":''), 'good');
		else if (isset($level->errorMsg))
			$app->utils->message($level->errorMsg);
		else
			$app->utils->message('Invalid details: please make sure to check I confirm.');
	}
?>
</div>
