<?php
    ...
    if (is_numeric($_GET['edit'])) {
        $challenge = $app->challenges->getChallengeFromID($_GET['edit']);
        if (!$challenge) {
            $app->utils->message('Challenge not found');
            die();
        } else {
            if (isset($_POST['save'])) {
                $updated = $app->challenges->editChallengeForm($_GET['edit']);
    ...
    if ($_GET['form'] == 'html')
        ...
        $form_type = 'json';
    else if (isset($challenge->data['form']) && realpath($app->config('path') . '/files/elements/challenges/'.basename($challenge->data['form']).'.php'))
        $form_type = 'file';
?>
...
?>
    <div class='clr'>
        ...
        <input name="form" type="text" class="span_11" value="<?=isset($challenge->data['form']) && realpath($app->config('path') . '/files/elements/challenges/'.basename($challenge->data['form']).'.php')?basename($challenge->data['form']):'';?>" />
    </div>
<?php
    else:
?>
