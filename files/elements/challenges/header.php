...
<?php 
	...
	if (isset($challenge->challenge_after_uri) && (strtolower($challenge->group) != 'plank' || $challenge->completed)):
?>
	<a class='right next-challenge' href='<?=$challenge->challenge_after_uri;?>'><i class='icon-caret-right'></i></a>
<?php elseif(isset($challenge->challenge_after_uri) && strtolower($challenge->group) == 'plank'): ?>
	<span class='right next-challenge dark hint--left' data-hint="You must complete plank challenges in order,&#10;but you can attempt any other challenge."><i class='icon-caret-right'></i></span>
<?php else: ?>
	<span class='right next-challenge dark'><i class='icon-caret-right'></i></span>
<?php endif; ?>

	<h1 class='no-margin'><?=ucwords($challenge->group . " challenge " . $challenge->name);?></h1>
...
<?php if (isset($challenge->online) && $challenge->online): ?>
	<br/><span class='strong <?=$challenge->online == 'online'?'green':'red';?>'>Challenge <?=$challenge->online;?></span><br/>
<?php endif; ?>
...
<?php
	...
	if (isset($challenge->attempt)) {
		if ($challenge->attempt === true)
			$app->utils->message('Challenge complete'.(isset($challenge->challenge_after_uri)?", <a href='$challenge->challenge_after_uri'>next challenge</a>":''), 'good');
		else if (isset($challenge->errorMsg))
			$app->utils->message($challenge->errorMsg);
		else
			$app->utils->message('failed to complete');
	}
?>
</div>
