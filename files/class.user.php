<?php
    ...
    class user {
        ...
        public function __construct($app) {
            ...
            // "user: Check if user is logging in <br/>";
            //Check if user is logging in
            if (isset($_GET['logout']) && $_GET['logout'] == $_SESSION['csrf_basic']) {
                $this->logout();
            }

            // "user: Check if user is registering in <br/>";
            ...
            // "user: Check if user is logged in <br/>";
            ...
            if (isset($_SESSION['uid'])) {
                ...
                //echo "user: Check if user is logging in <br/>";
                //Check if user is logging in
            ...
            // "user: Check if user is using Google Auth code <br/>";
            ...
            // "user: Login, register or connect via facebook <br/>";
            //Login, register or connect via facebook
            //echo "facebook: ".$_GET['facebook']."<br/>";
            ...
            //echo "facebook: ".$_GET['facebook']."<br/>";
            //echo "fbusername: ".$_GET['fbusername']."<br/>";
            if (isset($_GET['facebook'])) {
               $this->fboauth($_GET['facebook'],$_GET['fbusername']);
            }
        }
        ...
        public function get_details() {
            ...
            $st = $this->app->db->prepare('SELECT username, score,name,email,mobile, (oauth_id IS NOT NULL) as connected,
                    IFNULL(site_priv, 1) as site_priv, IFNULL(pm_priv, 1) as pm_priv, IFNULL(forum_priv, 1) as forum_priv, IFNULL(pub_priv, 1) as pub_priv, IFNULL(member_priv, 1) as member_priv,member_till, verified, mobile_verified, IFNULL(`posts`.posts, 0) AS `posts`,
                    profile.gravatar, profile.img as `image`,
                    activity.consecutive, activity.consecutive_most, activity.joined,avail.gdlink
                    FROM users u
                    ...
                    LEFT JOIN avail_list avail
                    ON u.username=avail.user
            ...
            $this->karma_priv = 0;
            $st = $this->app->db->prepare('SELECT medals.medal_id, medals.colour_id, medals.label FROM medals INNER JOIN users_medals ON medals.medal_id = users_medals.medal_id WHERE (label = :label1 OR label = :label2 OR label = :label3) AND users_medals.user_id = :uid');
            $st->execute(array(':uid' => $this->uid, ':label1' => 'donator', ':label2' => 'karma', ':label3' => 'vip'));
            $res = $st->fetchAll();
            foreach($res AS $medal) {
                ...
                if (strcasecmp($medal->label, 'vip') === 0){
                    $this->vip = true;
                }
            }
        ...
        private function checkLogin($username, $password) {
           ...
           //if username not valid return false
            if (!$this->app->utils->check_user($username)) return false;
            $encodedPassword = "{SHA}" . base64_encode(pack("H*", sha1($password)));
            //$this->app->log->add("users", "checkLogin encodedPassword: ". $encodedPassword);
            
            $st = $this->app->db->prepare('SELECT user_id,user_password FROM users_ldap WHERE user_name = :username AND user_password = :password');
            $st->execute(array(':username' => $username,':password' => $encodedPassword));
            $row = $st->fetch();
            //$this->app->log->add("users", "checkLogin user_password: ". $row->user_password);
            if ($row && $row->user_password === $encodedPassword) {
                // Login user
                $this->uid = $row->user_id;
                //$this->app->log->add("users", "checkLogin uid: ". $this->uid);
            }
            if (!$this->uid) {
                return false; // No user_id defined, or invalid
            }
            
            return $this->uid; // Login successful
        }

        public function login($user, $pass) {
            //$ldapID = $this->app->ldap->checkLogin($user, $pass);
            $ldapID=$this->checkLogin($user, $pass);
            if ($ldapID) {            
            ...
            if ($this->loggedIn) {
                ...
                $this->createUser($this->uid, $user, $pass);
            }

            return $this->loggedIn;
        }
        
        public function fboauth($uid,$username) {
            // Login user
                        $this->loggedIn = true;
                        $this->uid = $uid;
                        ...
                        // Add to log
                        $this->app->log->add('users', 'Register [FB] [' . $username . ']');
        ...
        private function regenerateRememberToken() {
            ...
            setcookie('autologin', $token, time()+60*60*24*7, '/', 'crushit.fit', true, true);
        }
        
        private function createUser($user_id,$user_name,$user_password){
            if (!$this->app->utils->check_user($user_name)) return false;
            
            $encodedPassword = "{SHA}" . base64_encode(pack("H*", sha1($user_password)));
            $this->app->log->add("users", "createUser user_id: ". $user_id ." user_name: ".$user_name." user_password: ". $encodedPassword);

            $st = $this->app->db->prepare('INSERT INTO users_ldap (`user_id`, `user_name`,`user_password`) VALUES (:user_id, :user_name, :user_password)');
            $result = $st->execute(array(':user_id' => $user_id, ':user_name' => $user_name, ':user_password' => $encodedPassword));
            
            $this->app->log->add("users", "result: $result");
        ...
        public function register($username=null, $password=null, $email=null , $mobile=null) {
            // "register func <br/>";
            $this->app->log->add('users.log', 'function register() start');
            if (!$username) {
            ...
            if (!$this->app->utils->check_mobile($mobile))
            ...
            $st = $this->app->db->prepare('SELECT username FROM users WHERE mobile=?');
            $st->bindParam(1, $mobile);
            $st->execute();
            if ($st->fetch(PDO::FETCH_ASSOC))
            ...
            // Add to DB
            $st = $this->app->db->prepare('INSERT INTO users (`username`, `password`, `email`, `mobile`)
                    VALUES (:u, :p, :e, :m)');
            $result = $st->execute(array(':u' => $username, ':p' => $hash, ':e' => $email, ':m' => $mobile));
            ...
            $this->app->log->add("users", "uid: $uid");

            // Add to LDAP
            //$this->app->ldap->createUser($uid, $username, $password);
            if(!$this->createUser($uid, $username, $password)) return "Error creating Username";
            
            $this->app->log->add("users", "uid: $uid, username: $username, password: $password");
            ...
            $this->app->log->add('users', 'Send email');
            // Send email
            $this->sendVerficationEmail(true);
            
            // Send Mobile
            $this->sendVerficationMobile(true);
            
            $this->app->log->add('users', 'Add to feed');
            // Add to feed
            $this->app->feed->call($username, 'join');

            $this->app->log->add('users', 'Setup GA event');
            // Setup GA event
            ...
            $st = $this->app->db->prepare('INSERT INTO users_registration (`user_id`, `ip`) VALUES (:u, :i)');
        ...
        public function logout() {
            ...
            if (isset($_COOKIE['autologin'])) {
                ...
                setcookie('autologin', '', time()-1000, '/', 'crushit.fit', true, true);
            }
        ...
        public function update($changes) {
            ...
            // Mobile
            if ($changes['mobile'] && $this->app->utils->check_mobile($changes['mobile'])) {
                $st = $this->app->db->prepare('UPDATE users SET mobile = :mobile WHERE user_id = :uid');
                $changed = $st->execute(array(':mobile' => $changes['mobile'], ':uid' => $this->uid));
            ...
            $updates['show_mobile'] = (isset($changes['display_mobile'])?'1':'0');
        ...
        public function request($user) {
            ...
            // Send email
            $body = "We received a request for your CrushIt!! account details.<br/><br/>Username: {$row->username}<br/>To reset your password, click on this link: <a href='http://www.crushit.fit/?request={$token}'>http://www.crushit.fit/?request={$token}</a><br/><br/>If you feel you have received this message in error, delete this email. Your password can only be reset via this email.";
        ...
        public function changePassword($pass, $pass2, $uid = null) {
            ...
            if ($status) {
                ...
                $this->changePasswordLdap($this->username, $pass);
        ...
        private function changePasswordLdap($username,$newPassword){
            if (!$username || !$newPassword) return false;
        
            if (!$this->app->utils->check_user($username)) return false;
        
            // Set password  
            $newencodedPassword = "{SHA}" . base64_encode(pack("H*", sha1($newPassword)));
            
            $st = $this->app->db->prepare('UPDATE users_ldap SET user_password = :newencodedPassword WHERE user_name = :username LIMIT 1');
            $status = $st->execute(array(':username' => $username, ':newencodedPassword' => $newencodedPassword));
        ...
        public function sendVerficationEmail($new=false) {
            //$this->app->log->add("users.log", "inside sendVerficationEmail()");
            //echo "start sendVerficationEmail() <br/>";
            $token = md5(openssl_random_pseudo_bytes(32));
            $this->setData('verification', $token, $this->uid, true);

            // Send email
             $body = "Click on the following link to verify your e-mail address:<br/><a style='color:#ffffff; text-decoration: none;' href='https://www.crushit.fit/settings/account.php?verify={$token}'>https://www.crushit.fit/settings/account.php?verify={$token}</a>";

             if ($new) {
                 $body = "Thank you for signing up for a <a style='color:#ffffff; text-decoration: none;' href='https://www.crushit.fit/'>CrushIt!!</a> account.<br/><br/>" . $body;
             }
            ...
            //echo "calling mandrillSend() <br/>";
            //echo "email: ".$this->email."<br/>";
            //echo "data: ".$data."<br/>";
            
            //$this->app->email->mandrillSend($this->email, null, 'verify-email', 'Confirm your email address', $data);

            //echo "end sendVerficationEmail() <br/>";
            
            return true;
        }
        
        public function sendVerficationMobile($new=false) {
            $token = md5(openssl_random_pseudo_bytes(32));
            $this->setData('mobile_verification', $token, $this->uid, true);

            // Send email
             $sms = "Open the following link to verify your mobile number: https://www.crushit.fit/settings/account.php?mobile_verify={$token}";
        ...
        public function confirmVerificationMobile($code) {
            if ($this->checkData("mobile_verification", $code)) {
                $st = $this->app->db->prepare('UPDATE users SET mobile_verified = 1 WHERE user_id = :uid LIMIT 1');
                $st->execute(array(':uid' => $this->uid));
                return true;
            } else {
                return false;
