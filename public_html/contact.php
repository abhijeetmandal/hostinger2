<?php
    ...
    $app->page->canonical = 'http://crushit.fit/contact';
            ...
            if ($app->checkCSRFKey("contact", $_POST['token'])) {
                ...
                if (!isset($error)) {
                    ...
                    if ($app->user->uid != $first->from) {
                        //If registered user
                        if ($first->username) {
                            // $body = "A reply has been added to a ticket you opened. To view the message please click the following link:<br/><a style='color:#ffffff; text-decoration: none;' href='https://www.crushit.fit/contact?view={$id}'>https://www.crushit.fit/contact?view={$id}</a>";

                            // Notify user
                            $app->notifications->add($first->from, 'mod_contact', $app->user->uid, $id);
                        } else {
                            // $body = "A reply has been added to a ticket you opened. To view the message please click the following link:<br/><a style='color:#ffffff; text-decoration: none;' href='https://www.crushit.fit/contact?view={$id}&email={$first->email}'>https://www.crushit.fit/contact?view={$id}&email={$first->email}</a>";
                        }
