...
//echo "mobile_verify: ".$_GET['mobile_verify']."<br/>";
    ...
    if (isset($_GET['password'])) {
        ...
        //$app->log->add("users", "sendVerficationEmail() start");
        //echo "sendVerficationEmail() start"."<br/>";
        if (empty($_GET['verify'])) {
            $verifySent = $app->user->sendVerficationEmail();
        } else {
            $verifyConfirmed = $app->user->confirmVerification($_GET['verify']);
        }
        //echo "sendVerficationEmail() end"."<br/>";
        //$app->log->add("users", "sendVerficationEmail() end");
    } else if (!$app->user->mobile_verified && isset($_GET['mobile_verify'])) {
        //https://www.crushit.fit/settings/account.php?mobile_verify=f328b25f89b5eac182fa8675fb7171ff
        if (empty($_GET['mobile_verify'])) {
            $verifyMobileSent = $app->user->sendVerficationMobile();
        } else {
            $verifyMobileConfirmed = $app->user->confirmVerificationMobile($_GET['mobile_verify']);
        }
        //echo "sendVerficationEmail() end"."<br/>";
        //$app->log->add("users", "sendVerficationEmail() end");
    }
...
<?php
        if ($row):
?>
        An SMS will be sent to your account containing the link to confirm your mobile number.<br/>
        If you have deleted or not recieved an SMS in 24 hours please click the button below.<br/>
<?php
        endif;
?>
        <a class='button clean' href='?mobile_verify'>
...
?>
    <div class='row'>
        <h3>Facebook</h3>
        Connect your CrushIt!! account to your facebook account. This will allow you to login to your account via the facebook button and releasing you from the burden of remembering another password. We will never post anything to your profile.<br/>
...
?>
    ...
    <div class='warning clr'>
        ...
        <label>Facing problem with deleting your account please email us at hello@crushit.fit with your verified email-id. Action will be taken within 72 hours.</label><br/>
    </div>
