<?php
    ...
    if ($thread_page == 1) {
        $app->page->canonical = 'https://www.crushit.fit/forum/'.$thread->slug;
    } else {
        $app->page->canonical = 'https://www.crushit.fit/forum/'.$thread->slug.'?page='.$thread_page;
    }

    if ($thread_page < $thread_page_count) {
        $app->page->next = 'https://www.crushit.fit/forum/'.$thread->slug.'?page='.($thread_page + 1);
    }

    if ($thread_page > 1) {
        $app->page->prev = 'https://www.crushit.fit/forum/'.$thread->slug.'?page='.($thread_page - 1);
    }
